...
**设计风格：Light Courseware · 编辑式排版**
- 教育性、清晰、有质感的浅色课件风格
- 字体层次丰富：标题用衬线（h1-h4），正文用无衬线，公式用等宽字体
- 色彩以白底、浅蓝、浅黄和浅绿为主色系，避免大面积黑色或深色背景
...
- 前端使用 KaTeX 渲染，只有被 `$` 包裹的公式才会被正确渲染
- ✅ 正确：`$(-2, 1)$`、`$x$ 轴`、`$y=2x+1$`、`$\rightarrow$`、`$\text{{左2 + 上1}}$`
- ❌ 错误：`(-2, 1)`、`x轴`、`y=2x+1`、`ightarrow`、`\text{{左2 + 上1}}`（裸露的LaTeX代码不会被渲染）
